...
object nivel1 {
	...
	method inicializarNivel1() {
		...
		self.inicialziarProtagonista()
		
		
		
		keyboard.up().onPressDo({ pepita.mover(arriba) })
		//bloque: ejecuta el comando dentro cuando se presiona la tecla
		...
		game.onTick(1000, "caer", { pepita.cae() })
	}
	
	method inicialziarProtagonista() {
		pepita.position(game.at(0, 1))
		pepita.energia(100)
	}
}